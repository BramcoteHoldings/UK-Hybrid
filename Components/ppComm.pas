{ RRRRRR                  ReportBuilder Class Library                  BBBBB
  ...
  RR  RR                                                               BB   BB
  RR   RR                   Copyright (c) 1996-2011                    BBBBB   }
...
type
  ...
  TppCommunicator = class(TComponent, IUnknown)
    private
      FComponentDesigner: IppComponentDesigner;
    ...
    protected
      function GetComponentDesigner: IppComponentDesigner; virtual;
      ...
      procedure SetComponentDesigner(const aComponentDesigner: IppComponentDesigner); virtual;
      procedure DSChanged(aDesignStateSet: TppDesignStates); virtual;
      function GetDesignState: TppDesignStates; virtual;
    ...
    public
      ...
      property ComponentDesigner: IppComponentDesigner read GetComponentDesigner write SetComponentDesigner;
      property DesignState: TppDesignStates read GetDesignState;
...
begin
  ...
  if TraRTTI.IsValidPropName(Self, 'UserName') and (FUserName = '') then
...
procedure TppCommunicator.SetComponentDesigner(const aComponentDesigner: IppComponentDesigner);
begin
...
function TppCommunicator.GetComponentDesigner: IppComponentDesigner;
begin
...
function TppCommunicator.GetDesignState: TppDesignStates;
begin
  Result := FDesignState;
end;
...
begin
  ...
  for liIndex := 0 to liComponents-1 do
    if aOwner.Components[liIndex] is FComponentClass then
      AddComponent(TppCommunicator(aOwner.Components[liIndex]));

  {add components residing in data modules }
  for liIndex := 0 to Screen.DataModuleCount-1 do
    BuildListFromDataModule(Screen.DataModules[liIndex]);

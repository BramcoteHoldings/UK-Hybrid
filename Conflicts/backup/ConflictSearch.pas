...
type
  ...
  TfrmConflictSearch = class(TForm)
    ...
    vMattersTextValue: TcxGridDBColumn;
...
procedure TfrmConflictSearch.pageSearchChange(Sender: TObject);
begin
...
procedure TfrmConflictSearch.dbgrMatters1DblClick(Sender: TObject);
begin
  aButtonOk.Execute();
end;

procedure TfrmConflictSearch.MakeSQL(bSearch: boolean);
var
...
//      childSQL := childSQL + 'AND MATTER.DEBTORSTATUS = DEBTORSTATUS.DEBTORSTATUS(+) ';
   ...
   dmAxiom.qryMSearch.SQL.Text := dmAxiom.qryMSearch.SQL.Text + sOrderBy;
	...
	dmAxiom.qryMSearch.SQL.SaveToFile('msearch.sql');
...
procedure TfrmConflictSearch.DoSetParameters(DataSet: TUniQuery; DataSetTotal: TUniQuery);
begin
   if chkOpenedSince.Checked then
   begin
...
//   'DEBTORSTATUS.DESCRIPTION, DEBTORSTATUS.ROW_COLOUR, archivereview, archivedest, archivebox, matter.dummy  ';
   ...
   ' NVL ((SELECT bill_amount FROM (SELECT nfee_debtornotes, bill_amount, nmatter, '+
   ' MAX(nfee_debtornotes) OVER (PARTITION BY nmatter) AS rmax_nfee_debtornotes '+
   ' FROM fee_debtornotes) WHERE nfee_debtornotes = rmax_nfee_debtornotes AND nmatter = matter.nmatter), 0)  AS bill_amount ';
...
//      StringPopulate(cbWorkFlowType.Items, 'WORKFLOWTYPE', 'CODE');
   ...
   if SystemString('LOCALE_NAME') = 'United Kingdom' then
   begin
...
procedure TfrmConflictSearch.setDisplayNames;
var
   imaxLength : integer;
begin
   imaxLength := 0;
...
function TfrmConflictSearch.FindMatter(sFileID : string) : boolean;
begin
  sLastFileID := sFileID;
...
procedure TfrmConflictSearch.cbPartnerKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmConflictSearch.MakeTitle;
begin
   sTitle := ' Matter ';
   sAnd := ' for ';
...
procedure TfrmConflictSearch.AddTitle(sDescr : string; sValue : string);
begin
   if sValue <> '' then
   begin
...
procedure TfrmConflictSearch.FormMouseWheelDown(Sender: TObject;
...
procedure TfrmConflictSearch.FormMouseWheelUp(Sender: TObject;
  Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
//  if dbgrMatters.Visible then
//    dmAxiom.qryMSearch.MoveBy(-5);
...
procedure TfrmConflictSearch.FilterItemChange(Sender: TObject);
begin
  SaveFilter := True;
  bNoFilter := false;
  RefreshSearch := True;
...
procedure TfrmConflictSearch.cbBranchChange(Sender: TObject);
begin
{  SaveFilter := True;
  bNoFilter := false;
...
procedure TfrmConflictSearch.SetReopenList;
var
  LAction: TAction;
  i: Integer;
begin
...
procedure TfrmConflictSearch.cbWorkFlowTypeChange(Sender: TObject);
begin
  SaveFilter := True;
  RefreshSearch := True;
...
procedure TfrmConflictSearch.chkArchivedClick(Sender: TObject);
begin
  SaveFilter := True;
  bNoFilter := false;
...
procedure TfrmConflictSearch.MatterArchiveInit;
begin
...
procedure TfrmConflictSearch.cbJurisdictionChange(Sender: TObject);
begin
   SaveFilter := True;
   bNoFilter := false;
   RefreshSearch := True;
...
procedure TfrmConflictSearch.cbStatusChange(Sender: TObject);
begin
  SaveFilter := True;
  bNoFilter := false;
  RefreshSearch := True;
...
procedure TfrmConflictSearch.cbSubType1Change(Sender: TObject);
begin
  SaveFilter := True;
  bNoFilter := false;
...
procedure TfrmConflictSearch.aRefreshExecute(Sender: TObject);
begin
  dmAxiom.qryMSearch.Close;
  dmAxiom.qryMSearch.Open;
  pageSearch.ActivePage := tabGrid;
...
procedure TfrmConflictSearch.aWIPNotesExecute(Sender: TObject);
var
  LDateStart,
  LDateEnd : TDateTime;
  lsUserName : String;
...
procedure TfrmConflictSearch.aClearAllExecute(Sender: TObject);
begin
...
procedure TfrmConflictSearch.aMergeExecute(Sender: TObject);
var
  sTmpSQL : string;
  LWriteMerge: TfrmWriteMerge;
begin
  LWriteMerge := TfrmWriteMerge.Create(Self);
  sTmpSQL := 'SELECT OPENED, MATTER.PARTNER, AUTHOR, RATE, FILEID, MATTER.TITLE AS CLIENT, PARTY1, PARTY2, PARTY3, SHORTDESCR, '+
             'LONGDESCR, OFFICE, TRUST_BAL, CL_TRUST_BAL, BILL_TO, LASTTRANS, LASTTRUST, PHONEBOOK.REFERREDBY, DEBTORS, NOTES, '+
             'MATTER.CONTACT AS MATTER_CONTACT, EXP_COMPLETION, OPREF, PERIOD0, PERIOD1, PERIOD2, PERIOD3, MATTER.COMPLETED AS MATTER_COMPLETED,'+
             'MATTER.ARCHIVED as MATTER_ARCHIVED, MATTER.BARCODE, PHONEBOOK.* '+
...
begin
   try
      ...
      DoSetParameters(qryConflicts, qryTotal);
      rptMatterList.Print;
   end;
...
procedure TfrmConflictSearch.aPrintLabels(Sender: TObject);
...
procedure TfrmConflictSearch.aPrintStatusExecute(Sender: TObject);
begin
...
procedure TfrmConflictSearch.aButtonOkExecute(Sender: TObject);
begin
  bnOk.Click();
end;
...
procedure TfrmConflictSearch.aRecentHolderExecute(Sender: TObject);
var
  LFileID: string;
begin
...
procedure TfrmConflictSearch.rptMatterStatusPreviewFormCreate(
  Sender: TObject);
begin
...
procedure TfrmConflictSearch.ppVariable4Calc(Sender: TObject;
  var Value: Variant);
begin
   Value := Value + 1;
end;

procedure TfrmConflictSearch.cbTypeChange(Sender: TObject);
begin
   SaveFilter := True;
   bNoFilter := false;
   RefreshSearch := True;
...
procedure TfrmConflictSearch.cbSubTypeChange(Sender: TObject);
begin
   SaveFilter := True;
   bNoFilter := false;
   RefreshSearch := True;
...
procedure TfrmConflictSearch.cbTypeDropDown(Sender: TObject);
begin
   if not qryMatterType.Active then
      qryMatterType.Open();
end;

procedure TfrmConflictSearch.cbSubTypeDropDown(Sender: TObject);
begin
   if not qryMatterSubType.Active then
      qryMatterSubType.Open();
end;

procedure TfrmConflictSearch.cbPartnerDropDown(Sender: TObject);
begin
   if not qPartner.Active then
      qPartner.Open();
end;

procedure TfrmConflictSearch.cbControllerDropDown(Sender: TObject);
begin
   if not qController.Active then
      qController.Open();
end;

procedure TfrmConflictSearch.cbAuthorDropDown(Sender: TObject);
begin
   if not qAuthor.Active then
      qAuthor.Open();
end;

procedure TfrmConflictSearch.cbOperatorDropDown(Sender: TObject);
begin
...
procedure TfrmConflictSearch.cbOperatorEnter(Sender: TObject);
begin
   if not qOperator.Active then
      qOperator.Open();
end;

procedure TfrmConflictSearch.cbAuthorEnter(Sender: TObject);
begin
   if not qAuthor.Active then
      qAuthor.Open();
end;

procedure TfrmConflictSearch.cbControllerEnter(Sender: TObject);
begin
   if not qController.Active then
      qController.Open();
end;

procedure TfrmConflictSearch.cbPartnerEnter(Sender: TObject);
begin
   if not qPartner.Active then
      qPartner.Open();
end;

procedure TfrmConflictSearch.cbWorkFlowTypeDropDown(Sender: TObject);
begin
...
procedure TfrmConflictSearch.vMattersAUTHORStylesGetHeaderStyle(
...
procedure TfrmConflictSearch.vMattersCanSelectRecord(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  var AAllow: Boolean);
begin
aWIPNotes.Enabled := True;
...
procedure TfrmConflictSearch.vMattersColumnHeaderClick(
...
procedure TfrmConflictSearch.tbFileSearchChange(Sender: TObject);
begin
  EnableTimer(Sender);
  FLookup := luFileID;
end;

procedure TfrmConflictSearch.tbClientSearchChange(Sender: TObject);
begin
   EnableTimer(Sender);
   FLookup := luClient;
end;

procedure TfrmConflictSearch.dxBarSubItem2Popup(Sender: TObject);
begin
   ReopenListSet('MATTER', dxBarMRUListMatters);
end;

procedure TfrmConflictSearch.edBoxNumberPropertiesChange(Sender: TObject);
begin
   rgMatterOptions.ItemIndex := 3;
   FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.edtBarcodePropertiesChange(Sender: TObject);
begin
   rgMatterOptions.ItemIndex := 3;
...
procedure TfrmConflictSearch.edtBarcodeSearchPropertiesChange(Sender: TObject);
begin
   rgMatterOptions.ItemIndex := 3;
...
procedure TfrmConflictSearch.cbTypePropertiesInitPopup(Sender: TObject);
begin
   if not qryMatterType.Active then
      qryMatterType.Open();
end;

procedure TfrmConflictSearch.cbSubTypePropertiesInitPopup(Sender: TObject);
begin
   if not qryMatterSubType.Active then
      qryMatterSubType.Open();
end;

procedure TfrmConflictSearch.cbBranchPropertiesInitPopup(Sender: TObject);
begin
...
procedure TfrmConflictSearch.cbJurisdictionPropertiesInitPopup(
  Sender: TObject);
begin
...
procedure TfrmConflictSearch.cbStatusPropertiesInitPopup(Sender: TObject);
begin
...
procedure TfrmConflictSearch.cmbDepartmentPropertiesInitPopup(
  Sender: TObject);
begin
   if not qryDepartment.Active then qryDepartment.Open;
end;

procedure TfrmConflictSearch.cmbDepartmentPropertiesChange(Sender: TObject);
begin
   SaveFilter := True;
   bNoFilter := false;
   RefreshSearch := True;
...
procedure TfrmConflictSearch.rgMatterOptionsClick(Sender: TObject);
begin
   RefreshSearch := True;
end;

procedure TfrmConflictSearch.cxLookupComboBox1PropertiesInitPopup(
  Sender: TObject);
begin
   if not qryWorkFlowType.Active then
      qryWorkFlowType.Open;
...
procedure TfrmConflictSearch.tabFiltersShow(Sender: TObject);
begin
   if SystemString('docs_reg') <> C_DOCUMENTPASSWORD then
      bbtnMSWord.Enabled := False
end;

procedure TfrmConflictSearch.bbtnMSWordClick(Sender: TObject);
var
...
procedure TfrmConflictSearch.cmbParaLegalEnter(Sender: TObject);
begin
   if not qParaLegal.Active then
      qParaLegal.Open();
end;

procedure TfrmConflictSearch.vMattersCustomDrawCell(
...
procedure TfrmConflictSearch.vMattersPARTNERStylesGetHeaderStyle(
  Sender: TcxGridTableView; AColumn: TcxGridColumn; var AStyle: TcxStyle);
var
   imaxLength : integer;
begin
...
procedure TfrmConflictSearch.aPrintGridExecute(Sender: TObject);
begin
   try
...
procedure TfrmConflictSearch.chkOpenedSinceClick(Sender: TObject);
begin
  dtpOpenedSince.Enabled := chkOpenedSince.Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkOpenedBeforeClick(Sender: TObject);
begin
  dtpOpenedBefore.Enabled := chkOpenedBefore.Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkCompletedSinceClick(Sender: TObject);
begin
  dtpCompletedSince.Enabled := chkCompletedSince.Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkCompletedBeforeClick(Sender: TObject);
begin
  dtpCompletedBefore.Enabled := chkCompletedBefore.Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkSLESinceClick(Sender: TObject);
begin
  dtpSLESince.Enabled := chkSLESince.Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkSLEBeforeClick(Sender: TObject);
begin
  dtpSLEBefore.Enabled := chkSLEBefore.Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkArchivedDateClick(Sender: TObject);
begin
...
procedure TfrmConflictSearch.chkArchivedDateToClick(Sender: TObject);
begin
...
procedure TfrmConflictSearch.chkArchivedSinceClick(Sender: TObject);
begin
  dtpArchivedSince.Enabled := (Sender as TcheckBox).Checked;
  FilterItemChange(Sender);
end;

procedure TfrmConflictSearch.chkArchivedBeforeClick(Sender: TObject);
begin

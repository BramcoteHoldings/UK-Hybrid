...
uses
  ...
  Dialogs, cxPC, cxControls, DB, MemDS, DBAccess, Ora, OracleUniProvider, Uni,
  ...
  cxCalendar, cxCurrencyEdit, cxGraphics, cxPCdxBarPopupMenu, cxLookAndFeels,
  cxLookAndFeelPainters, dxCore, cxDateUtils, dxPSGlbl, dxPSUtl, dxPSEngn,
  dxPrnPg, dxBkgnd, dxWrap, dxPrnDev, dxPSCompsProvider, dxPSFillPatterns,
  dxPSEdgePatterns, dxPSPDFExportCore, dxPSPDFExport, cxDrawTextUtils,
  dxPSPrVwStd, dxPSPrVwAdv, dxPSPrVwRibbon, dxPScxPageControlProducer,
  dxPScxEditorProducers, dxPScxExtEditorProducers, dxPSCore, dxPSBaseGridLnk,
  dxPSLVLnk, cxClasses, dxPScxGridLnk, dxPScxGridLayoutViewLnk;
...
const RPTFIELDS: array[0..14] of string = ('DESCR', 'ACTION_DATE', 'REFNO', 'ASSIGNED_TO', 'ALERT_TO', 'ALERT_TO_ADD', 'FILEID','BILL_TO', 'NAME', 'SHORTDESCR',  'DEBTOR_PHONE', 'AUTHOR', 'NOTES','template_descr','BALANCE');
const RPTDESCR: array[0..14] of string = ('Action', 'Action Date', 'Bill No', 'Assign To', 'Alert To', 'Add Alert To', 'File ID', 'Bill To', 'Client', 'Matter Descr', 'Debtor Phone#', 'Author', 'Notes','Template','Balance');
...
type
  ...
  private
    ...
    procedure addColumn(lList: TListView; var lNode: TListItem; sCaption: string; iImageIndex: integer);
    procedure saveGrid;
    procedure loadGrid;
  public
    { Public declarations }
...
procedure TfrmDebtorTaskDue.addColumn(lList: TListView; var lNode: TListItem; sCaption: string; iImageIndex: integer);
var iSubItem: integer;
begin
  if lNode = nil then
  begin
...
begin
  ...
  qryDueItems.SQL.Add('alert_to,trunc(action_date) as action_date,ta.document_path,dt.confirmed_Date,dt.processed_date,memo.refno,ma.shortdescr, alert_to_add ');
...
begin
   ...
   //      end;
      ...
      merg.wordMerge(lData^.nMemo,
         ...
         qryDueItems.fieldByname('DESCR').AsString,
         lData^.itaskKey,
         dxAutoWord.Down,dxShow.Down,dxPrint.Down);
      if (dxAutoWord.Down = False) then
         merg.ShowModal;
...
var iLoop, iNumberCols, iColPos: integer;
  lCol: TcolumnDetails;
  lNewCol: TListColumn;

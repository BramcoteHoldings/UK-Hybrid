...
uses
  ...
  Db, StdCtrls, Buttons, ComCtrls, OraSmart, MemDS, Ora, OracleUniProvider, Uni,
  DBAccess;
...
begin
  ...
  begin
    ...
    cbStatus.ItemIndex := qryTask.FieldByName('STATUS').AsInteger;
...
begin
  ...
  begin
    with qryTask do
    begin
      SQL.Text := 'UPDATE TASK SET CODE = :CODE, FILEID = :FILEID, DESCR = :DESCR, '+
                  'AUTHOR = :AUTHOR, COMMENCE = :COMMENCE, DUE = :DUE, NMATTER = :NMATTER, STATUS = :STATUS '+
                  'WHERE NTASK = :NTASK';
      ...
      ParamByName('NMATTER').AsString := MatterString(lblMatter.Caption, 'NMATTER');
...
procedure TfrmTaskNew.FormShow(Sender: TObject);
begin
   if (FFileID <> '') then

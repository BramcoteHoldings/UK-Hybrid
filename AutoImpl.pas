...
uses
  Windows, Classes, ActiveX, Word2000;
...
type
  ...
  public
    ...
    procedure RunMacro(AMacro: string);
    procedure MergeDoc(ADataFilePath: string);
  published
...
procedure TWordObject.RunMacro(AMacro: string);
var
  RouteDocument  : OleVariant;
begin
  RouteDocument := UnAssigned;
  OleVariant(FWordApp).ActiveDocument.Run(AMacro);
end;

procedure TWordObject.MergeDoc(ADataFilePath: string);
var
  RouteDocument  : OleVariant;
begin
  RouteDocument := UnAssigned;
  OleVariant(FWordApp).MailMerge.OpenDataSource(ADataFilePath,,True);
  OleVariant(FWordApp).MailMerge.Destination := wdSendToNewDocument;
  OleVariant(FWordApp).MailMerge.Execute();
end;

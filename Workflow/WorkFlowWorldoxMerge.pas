unit WorkFlowWorldoxMerge;


interface

uses Classes, SysUtils, db, Windows,VirtualTable,
     WorkFlowDocumentMerge , WDAPI32;
...
type
  TWorldoxMergeProperties = (
    inSaveDestination,
    inInformPath);

  TWorldoxMergeDocument = class(TMergeDocument)
  private
...
//    class function GetPropertyValue(AProperty: String): Variant; override;
  ...
  TWorldoxRepeatingParties = class(TCollection)
  public
    constructor Create();
...
class function TWorldoxMergeDocument.ExportDescription: String;
begin
  Result := 'Worldox Precedents';
end;

function TWorldoxMergeDocument.GenerateDataFile: boolean;
var
   LDataFilePath,
   LFieldValue,
   LFieldName,
   ...
   LRepeatPartys: TWorldoxRepeatingParties;
...
class procedure TWorldoxMergeDocument.LoadSettings;
begin
  // do nothing
end;

class function TWorldoxMergeDocument.NumberOfProperties: Integer;
begin
  Result := 0;
end;

class function TWorldoxMergeDocument.PropertyType(AProperty: Integer): TMergePropertyType;
begin
  Result := mptNone;
end;

class function TWorldoxMergeDocument.PropertyType(AProperty: String): TMergePropertyType;
begin
  Result := mptNone;
end;

class procedure TWorldoxMergeDocument.SetPropertyValue(AProperty: Integer;
  AValue: Variant);
begin
  // DO NOTHING
end;

class procedure TWorldoxMergeDocument.SaveSettings;
begin
  // do nothing.
end;

class procedure TWorldoxMergeDocument.SetPropertyValue(AProperty: String;
  AValue: Variant);
begin
// DO NOTHING.
end;

procedure TWorldoxMergeDocument.WriteData;
var
{  LNew: Boolean;
  Rec: widestring;
  s1,s2,s3,s4, LAnswerName: WideString;
...
function TWorldoxMergeDocument.MergeDocument: integer;
var
...
class function TWorldoxMergeDocument.Options: TMergeOptions;
begin
...
procedure TWorldoxMergeDocument.FindOldValues(ACustomFieldInfo: TCustomFieldInfo);
var
  LFieldName: String;
...
constructor TWorldoxMergeDocument.Create(Collection: TCollection);
begin
...
destructor TWorldoxMergeDocument.Destroy;
begin
  FAnswers.Free();
  inherited;
end;

function TWorldoxMergeDocument.PostProcess(AChangedDataValues: TCollection): boolean;
begin
  Result := True;
end;


procedure TWorldoxRepeatingParties.AddFieldValue(APartyType, AField, AValue: String; AFieldType: TFieldType);
var
...
constructor TWorldoxRepeatingParties.Create;
begin
...
initialization
  RegisterWorkflowMergeType(TWorldoxMergeDocument);

unit WorkFlowToDoReportSequence;
...
type
  TqrWorkFlowToDoReportSequence = class(TQuickRep)
...
var
  qrWorkFlowToDoReportSequence: TqrWorkFlowToDoReportSequence;
...
procedure TqrWorkFlowToDoReportSequence.QuickRepBeforePrint(
...
procedure TqrWorkFlowToDoReportSequence.QuickRepAfterPrint(Sender: TObject);
begin
  if qryWorkFlowTaskReport.Active = True then
    qryWorkFlowTaskReport.Close();
end;

procedure TqrWorkFlowToDoReportSequence.DetailBandTasksBeforePrint(
...
procedure TqrWorkFlowToDoReportSequence.QRGroupMatterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
...
procedure TqrWorkFlowToDoReportSequence.TitleBandBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TqrWorkFlowToDoReportSequence.PageHeaderBand1BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  lEntity.Caption := SystemString('COMPANY');
end;

procedure TqrWorkFlowToDoReportSequence.tStartedPrint(sender: TObject;
  var Value: String);
begin
  Value := FormatDateTime('dd mmm yy',qryWorkFlowTaskReportSTARTED.AsDateTime);
end;

procedure TqrWorkFlowToDoReportSequence.tCompleteByPrint(sender: TObject;
  var Value: String);
begin
...
procedure TqrWorkFlowToDoReportSequence.Initialise(EmployeeCode: String; DateFrom, DateTo: TDate; AExcCritDate: boolean);
begin
...
procedure TqrWorkFlowToDoReportSequence.Initialise(TasksList: TStrings; EmployeeCode: String; DateFrom, DateTo: TDate);
var
  index : integer;
begin
  // Add selected tasks

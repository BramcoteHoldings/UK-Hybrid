...
uses
  ...
  Dialogs, cxPC, dxBarBuiltInMenu, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxContainer, cxEdit, cxStyles, cxCustomData, cxFilter,
  cxData, cxDataStorage, cxNavigator, dxDateRanges,
  cxDataControllerConditionalFormattingRulesManagerDialog, Data.DB, cxDBData,
  cxCheckBox, cxDBLookupComboBox, dxCore, dxCoreClasses, dxGDIPlusAPI,
  ...
  dxRichEdit.Control.Core, System.Actions, Vcl.ActnList,
  Vcl.PlatformDefaultStyleActnCtrls, Vcl.ActnMan, ppDB, ppDBPipe, ppParameter,
  ppRichTx, ppBands, ppClass, ppDesignLayer, ppModule, raCodMod, ppCtrls,
  ppReport, ppPrnabl, ppStrtch, ppSubRpt, ppCache, ppComm, ppRelatv, ppProd,
  Uni, Vcl.Menus, DBAccess, MemDS, System.ImageList, Vcl.ImgList, dxBar,
  dxBarExtDBItems, dxBarExtItems, cxBarEditItem, cxClasses, Vcl.StdCtrls,
  cxRadioGroup, cxGridLevel, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxGridCustomView, cxGrid, dxDockControl, cxLabel,
  Vcl.ExtCtrls, dxStatusBar, cxMemo, cxTextEdit, Vcl.ComCtrls, dxDockPanel,
  IdIOHandler, IdText, IdIOHandlerSocket, IdIOHandlerStack, IdSSL, IdSSLOpenSSL,
  IdBaseComponent, IdComponent, IdTCPConnection, IdTCPClient,
  IdMessageClient, IdSMTP, IdMessage, IdExplicitTLSClientServerBase, IdSMTPBase,
  Math, IdSASL, IdSASLUserPass, IdSASLLogin, dxRichEdit.Html,
  IdUserPassProvider, IdIntercept, IdLogBase, IdLogFile, Clipbrd;

const
   BULKMAILER_ClOSE = WM_APP + 405;
...
type
  TfrmBulkMailer = class(TForm)
    ...
    procedure actSendEmailsExecute(Sender: TObject);
    procedure actExportExecute(Sender: TObject);
    procedure actCloseExecute(Sender: TObject);
  protected
    procedure CloseFrm(var Message: TMessage); message BULKMAILER_ClOSE;
  private
...
//     HTMLEdit.LoadFromFile(FileName);
      ...
      dxRichEditControl1.LoadDocument(fileName);   //  HTMLEditor.LoadFromFile(FileName);
...
begin
   try
      ...
      try
         ...
         dxRichEditControl1.LoadDocument(tmpFileName, TdxRichEditDocumentFormat.Html);
...
procedure TfrmBulkMailer.actSendEmailsExecute(Sender: TObject);
var
   sOracleError: string;
   bOk: boolean;
   FHostname: string;
...
procedure TfrmBulkMailer.actExportExecute(Sender: TObject);
var
   lEmailList: string;
   nRowCount: integer;
...
procedure TfrmBulkMailer.actCloseExecute(Sender: TObject);
begin
   Release;
end;
...
begin
   PostMessage(Handle, BULKMAILER_ClOSE,0,0);//  Close;
end;
...
procedure TfrmBulkMailer.CloseFrm(var Message: TMessage);
begin
   Self.Release;
   inherited;
end;

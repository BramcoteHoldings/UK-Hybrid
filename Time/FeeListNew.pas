...
uses
  ...
  cxCalendar, cxCurrencyEdit, cxPC, cxSplitter, cxMemo, LMDCustomControl,
  LMDCustomPanel, LMDCustomBevelPanel, LMDSimplePanel, dxPSGlbl, dxPSUtl,
  dxPrnPg, dxBkgnd, dxWrap, dxPrnDev, dxPSEngn, dxPSCompsProvider,
  dxPSFillPatterns, dxPSEdgePatterns, dxPSCore, dxPScxCommon,
  dxPScxGridLnk, dxPgsDlg, ad3RichEditDB, cxGridChartView,
  ...
  dxPScxSchedulerLnk, ppDesignLayer, ppStrtch, ppMemo, dxBarExtItems, LMDControl,
  cxNavigator, cxRichEdit, System.Actions, cxBarEditItem;
...
type
  ...
  TfrmFeeListNew = class(TForm)
    ...
    procedure tsFilterShow(Sender: TObject);
...
//  dmFeeList.qFees.Debug := true;
  ...
  dmFeeList.qFees.SQL.Add('SELECT /*+ ORDERED */FEE.NFEE, /*+ ORDERED */trunc(FEE.CREATED) "CREATED" ');
  ...
  begin
    ...
    dmFeeList.qFees.SQL.Add(FOrderBy);
  end;
...
procedure TfrmFeeListNew.tsStatsShow(Sender: TObject);
var
...
begin
   ...
   if not VarIsNull(cbAuthor.EditValue) then
   begin
    lAuthorUnitsMoneyMsg.Caption := String(cbAuthor.EditValue) + ' $/Unit';
...
procedure TfrmFeeListNew.tsFeeListShow(Sender: TObject);
begin
   MakeSQL();
   grdFees.FocusedView.DataController.Groups.FullExpand;
...
procedure TfrmFeeListNew.tsFilterShow(Sender: TObject);
begin
...
begin
  ...
  if frmDesktop.pagMainControl.ActivePageIndex = 0 then
     frmDesktop.fdTab.AddForm(frmMatters,True);
  if (not frmMatters.Visible) then
     frmMatters.Show;
end;

...
uses
  ...
  cxCheckBox, cxImageComboBox, cxCalendar, cxButtonEdit, cxSpinEdit,
  cxCurrencyEdit, cxDropDownEdit, cxRichEdit, dxBarBuiltInMenu, Vcl.Menus,
  ppBands, ppClass, ppCtrls, ppMemo, cxGridCustomPopupMenu, cxGridPopupMenu,
  ppDB, ppDBPipe, ppParameter, ppStrtch, ppRegion, ppDesignLayer, ppModule,
  raCodMod, ppRichTx, ppVar, ppReport, ppSubRpt, ppPrnabl, ppCache, ppComm,
  ppRelatv, ppProd, dxBar, cxBarEditItem, dxBarExtItems, cxClasses, DBAccess,
  Uni, Vcl.ExtCtrls, MemDS, Vcl.ImgList, Vcl.StdCtrls, Vcl.ComCtrls,
  cxMemo, cxTextEdit, cxMaskEdit, cxGroupBox, cxGridLevel,
  cxGridBandedTableView, cxGridDBBandedTableView, cxGridCustomTableView,
  cxGridTableView, cxGridDBTableView, cxGridCustomView, cxGrid, cxLabel,
  cxRadioGroup, NumberEdit, Vcl.Buttons, Vcl.DBCtrls, Variants, ppFileUtils,
  ppIniStorage, DateUtils, cxGridExportLink, dxDPIAwareUtils, cxDBRichEdit,
  dxGDIPlusClasses, System.ImageList, dxDateRanges;
...
type
  TfrmInvoice = class(TForm)
    ...
    procedure neFeesKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure memoNotesKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure popNotesKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure memoNotesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure tvBillItemsEditKeyDown(Sender: TcxCustomGridTableView;
      AItem: TcxCustomGridTableItem; AEdit: TcxCustomEdit; var Key: Word;
      Shift: TShiftState);
    procedure tvBillItemsMouseDown(Sender: TObject; Button: TMouseButton;
    ...
    procedure tvBillItemsEditing(Sender: TcxCustomGridTableView;
    ...
    procedure tvBillItems1DESCRPropertiesValidate(Sender: TObject;
      var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
    procedure tvBillItems2InitEdit(Sender: TcxCustomGridTableView;
      AItem: TcxCustomGridTableItem; AEdit: TcxCustomEdit);
    procedure tvBillItemsDESCRPropertiesInitPopup(Sender: TObject);
    procedure tvBillItemsDESCRPropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure pbSpellCheckClick(Sender: TObject);
    procedure dxBarButton4Click(Sender: TObject);
    procedure cxRichEdit1KeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure tvBillItemsStylesGetContentStyle(Sender: TcxCustomGridTableView;
...
uses
  GenericSearch, MiscFunc, NSearch, InvPost, AxiomData, Desktop, FeeNew,
  CheqReqNew, SundryNew, citfunc, Matters,DebtorTaskNew,subBill, Bills,
  bill_narration, FeeListDataModule, FeeEditQuick, ShellAPI, BillTemplatePrint,
  EditText, billfeelist, MSearch, GenEditor, ExpTemplateSelection, glComponentUtil,
  DebtorNotes, AlterBilledTaxAmount;
...
begin
   ...
   IsBillItem := 'N';
//   IsBillHybrid := 'N';
//   IsBillHybrid := SystemString('ALLOW_HYBRID_BILL');
      ...
      begin
         ...
         begin
            ...
            lblUnProcessedTime.Caption := 'There is a total of '+ CurrToStrF(Unprocessed, ffCurrency, 2) +
                                          ...
                                          'If you would like to bill these Time transactions, please go to the TimeSheet and '+
                                          'process the transactions.'+chr(13)+lsAuthorList;
         end;
      end;
   end;
...
//   popNotes.Text := qryInvoice.FieldByName('INV_NOTE').AsString;
   ...
   if (qryInvoice.FieldByName('IS_DRAFT').AsString = 'N') and
      (boolean(cbAssessed.EditValue) = False) then
      Self.PopupMenu := pmForm;

   if neTrust.AsCurrency > 0 then btnAddTrust.caption := 'Remove Trust';
   ...
   if (HasWithholdingTaxForSelectedItem) then
     ...
     barbtnRemoveDiscount.Enabled := dmAxiom.Security.Bill.Discount.Remove and
                                   (qryInvoice.FieldByName('IS_DRAFT').AsString = 'N') and
                                   (qryInvoice.FieldByName('DISCOUNT').AsCurrency <> 0);


   if qryInvoice.FieldByName('DISPATCHED').AsString <> '' then
   begin
       if ((qryInvoice.FieldByName('RV_TYPE').AsString = 'N') or
          (qryInvoice.FieldByName('RV_TYPE').AsString = 'D')) and
          (qryInvoice.FieldByName('TAKE_ON').AsString = 'N') then
       begin
          if ((qryInvoice.FieldByName('FEES_PAID').AsCurrency +
             qryInvoice.FieldByName('DISB_PAID').AsCurrency +
 ...
 //       dtpInterim.Enabled := False;
   ...
   begin
     ...
     //rb Look at latter
      ...
      neTrust.Enabled := dmAxiom.Security.Bill.LockAmounts.ChangeTrust and
                         dmAxiom.Security.Bill.Post and dmAxiom.Security.Bill.TrustTransfer;
...
//     btnInterimDate.Enabled := dtpInterim.Checked;
   ...
   if (IsBillItem = '') and (qryInvoice.FieldByName('billtemplate').IsNull) then
   begin
      with dmAxiom.qryTmp do
      begin
         Close;
...
{      qryNew.SQL.Text := 'SELECT SUM(AMOUNT) AS TOT FROM FEE WHERE NMATTER = ' +
      ...
      qryNew.SQL.Text := 'SELECT SUM (amount) AS TOT FROM (SELECT NVL (amount, 0) AS amount '+
                         ' FROM fee WHERE nmatter IN (SELECT nmatter FROM matterassoc, matter '+
                         ' WHERE mainmatter = '+ QuotedStr(qryInvoice.FieldByName('FILEID').AsString) +
                          ' AND assocmatter = fileid) '+
                          ' AND ((nmemo IS NULL OR nmemo = 0) AND billed = ''N'' AND TYPE = ''N'' AND billtype = ''Billable'' '+
                          ' OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + '))'+
                          ' UNION ALL '+
                          ' SELECT NVL (amount, 0) AS amount FROM fee WHERE nmatter = ' +  qryInvoice.fieldByName('NMATTER').AsString +
                          ' AND ((nmemo IS NULL OR nmemo = 0) AND billed = ''N'' AND TYPE = ''N'' AND billtype = ''Billable'' '+
                          ' OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + ')))';
   end
   else
   begin
      ...
      begin
         qryNew.SQL.Text := 'SELECT SUM(AMOUNT) AS TOT, 0 as Unit_Total FROM FEE WHERE NMATTER = ' +
                        IntToStr(qryInvoice.fieldByName('NMATTER').AsInteger) +
                        ' AND ((NMEMO is null AND FEE.BILLED = ''N'' AND TYPE = ''N'' AND FEE.BILLTYPE = ''Billable'') '+
                        ' OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + '))';
      end
      else
      begin
         qryNew.SQL.Text := 'SELECT * FROM '+
                        '(SELECT SUM(amount) AS TOT FROM FEE WHERE NMATTER = ' +
                        IntToStr(qryInvoice.fieldByName('NMATTER').AsInteger) +
                        ' AND ((NMEMO is null AND FEE.BILLED = ''N'' AND TYPE = ''N'' AND FEE.BILLTYPE = ''Billable'') '+
                        ' OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + '))),' +
                        '(SELECT SUM(task_AMOUNT) as Unit_Total FROM FEE WHERE NMATTER = ' +
                        IntToStr(qryInvoice.fieldByName('NMATTER').AsInteger) +
                        ' AND ((NMEMO is null AND FEE.BILLED = ''N'' AND TYPE = ''N'' AND FEE.BILLTYPE = ''Billable'') '+
                        ' OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + ')))';
      end;
   ...
   if not qryNew.IsEmpty then
   begin
      lblFeesAvail.Caption := Format('%m', [qryNew.FieldByName('TOT').AsCurrency]);
      dFeesAvail := qryNew.FieldByName('TOT').AsCurrency;
      if (IsBillItem = 'Y') then
      begin
         lblUnitFeesAvail.Caption := Format('%m', [qryNew.FieldByName('UNIT_TOTAL').AsCurrency]);
   ...
   if (dFeesAvail > 0) and (qryInvoice.FieldByName('IS_DRAFT').AsString = 'Y') then
   begin
 ...
 //  qryNew.SQL.Add('AND ((BILLED = ''N'' AND TRUST <> ''T'' AND (NCHEQUE > 0 OR TYPE = ''J2'' OR TYPE = ''DR'')) OR (NRECEIPT = 0 AND NMEMO =' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) +'))');
   qryNew.SQL.Add('AND ((BILLED = ''N'' AND TRUST <> ''T'' AND (NCHEQUE > 0 OR TYPE = ''J2'' OR TYPE = ''DR'')) OR (NMEMO =' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) +'))');
   qryNew.SQL.Add('AND NINVOICE IS NULL');
   qryNew.Open;
   if not qryNew.IsEmpty then
     lblDisbAvail.Caption := Format('%m', [0 - qryNew.FieldByName('TOT').AsCurrency])
   else
   ...
   qryNew.SQL.Add('BILLED = ''N'' AND TRUST <> ''T'') OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) +'))');
 {
   ...
   qryNew.Open;
   if not qryNew.IsEmpty then
     lblUnPaidAvail.Caption := Format('%m', [0 - qryNew.FieldByName('TOT').AsCurrency])
   else
     lblUnPaidAvail.Caption := '0.00';
   // Anticipated Disbursements (CheqReqs) Available
   qryNew.Close;
   qryNew.SQL.Text := 'SELECT SUM(AMOUNT) AS TOT FROM CHEQREQ WHERE NMATTER = ' + qryInvoice.FieldByName('NMATTER').AsString +  // QuotedStr(TableString('MATTER', 'NMATTER', qryInvoice.FieldByName('NMATTER').AsInteger, 'FILEID')) +
                      ' AND ((NMEMO is null AND BILLED = ''N'' AND TRUST <> ''T'') OR '+
                      ' (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + '))';
   qryNew.Open;
   if not qryNew.IsEmpty then
     lblAntDAvail.Caption := Format('%m', [qryNew.FieldByName('TOT').AsCurrency])
   else
     lblAntDAvail.Caption := '0.00';
   // Sundries Available
   qryNew.Close;
   qryNew.SQL.Text := 'SELECT SUM(AMOUNT) AS TOT FROM SUNDRY WHERE NMATTER = ' + IntToStr(qryInvoice.fieldByName('NMATTER').AsInteger) + ' AND ((NMEMO is null AND BILLED = ''N'') OR (NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) + '))';
   qryNew.Open;
   if not qryNew.IsEmpty then
     lblSundAvail.Caption := Format('%m', [qryNew.FieldByName('TOT').AsCurrency])
   else
...
//   nlTrustAvail.Caption := Format('%m', [cTrustBal - cProtectedBal - cUnconCheqReq]);
   nlTrustAvail.Caption := Format('%m', [cTrustBal - cUnconCheqReq]);
   // AES 10/6/2005
   // trust should be clear less protected
//   if (cTrustBal - cProtectedBal) <= 0 then
//     neTrust.Enabled := False;

   if ((cTrustBal) <= 0) and (SystemString('EDIT_BILL_TRUST') = 'Y')
      AND (qryInvoice.FieldByName('IS_DRAFT').AsString = 'Y') then
      neTrust.Enabled := False;

   if qryInvoice.FieldByName('DISPATCHED').IsNull then
   begin
      if ((neFeesTax.AsCurrency = 0.0) or FFeeRebuilt) then
      begin
         neFeesTax.AsCurrency := ShowTax(neFees.AsCurrency, dGstFree, 'FEE',
                                         ...
                                         iMemo, qryInvoice.FieldByName('GENERATED').AsDateTime );
         TaxFees := neFeesTax.AsCurrency;
         neFeesTaxFree.AsCurrency := dGstFree;
//         sgrTotals.Cells[0, 0] := Format('%m', [neFeesTax.AsCurrency]);
         lblTotalFees.Caption := Format('%m', [neFeesTax.AsCurrency]);
         FFeeRebuilt := False;
      end;
      if ((neDisbTax.AsCurrency = 0.0) or FDisbsRebuilt) then
      begin
         neDisbTax.AsCurrency := ShowTax(neDisb.AsCurrency, dGstFree, 'ALLOC',
                                         qryInvoice.FieldByName('FILEID').AsString, DefaultTax,
                                         qryInvoice.FieldByName('NMATTER').AsInteger,
                                         iMemo, qryInvoice.FieldByName('GENERATED').AsDateTime);
...
//         sgrTotals.Cells[0, 1] := Format('%m', [neDisbTax.AsCurrency]);
         lblDisbTotal.Caption := Format('%m', [neDisbTax.AsCurrency]);
         FDisbsRebuilt := False;
      end;
      if ((neAntdTax.AsCurrency = 0.0) or FCheqReqRebuilt) then
      begin
         neAntdTax.AsCurrency := ShowTax(neAntD.AsCurrency, dGstFree, 'CHEQREQ',
                                         qryInvoice.FieldByName('FILEID').AsString, DefaultTax,
                                         qryInvoice.FieldByName('NMATTER').AsInteger,
                                         iMemo, qryInvoice.FieldByName('GENERATED').AsDateTime);
...
//         sgrTotals.Cells[0, 2] := Format('%m', [neAntdTax.AsCurrency]);
         lblAntdTotal.Caption :=  Format('%m', [neAntdTax.AsCurrency]);
         FCheqReqRebuilt := False;
      end;
      if ((neUpCredTax.AsCurrency = 0.0) or FCredRebuilt) then
      begin
         neUpCredTax.AsCurrency := ShowTax(neUpCred.AsCurrency, dGstFree, 'UPCRED',
                                          qryInvoice.FieldByName('FILEID').AsString, DefaultTax,
                                          qryInvoice.FieldByName('NMATTER').AsInteger,
                                          iMemo, qryInvoice.FieldByName('GENERATED').AsDateTime);
...
//         sgrTotals.Cells[0, 3] := Format('%m', [neUpCredTax.AsCurrency]);
         lblCredTotal.Caption := Format('%m', [neUpCredTax.AsCurrency]);
         FCredRebuilt := False;
      end;
      if ((neSundTax.Value = 0.0) or FSundRebuilt) then
      begin
         neSundTax.Value := ShowTax(neSund.AsCurrency, dGstFree, 'SUNDRY',
                                      qryInvoice.FieldByName('FILEID').AsString, DefaultTax,
                                      qryInvoice.FieldByName('NMATTER').AsInteger,
                                      iMemo, qryInvoice.FieldByName('GENERATED').AsDateTime);
...
//         sgrTotals.Cells[0, 4] := Format('%m', [neSundTax.Value]);
         lblSundTotal.Caption    := Format('%m', [neSundTax.Value]);
         FSundRebuilt := False;
      end;
      CalcTotal;
   end;
...
//  sgrTotals.Cells[0, 6] := Format('%m', [TaxFees + TaxDisb + TaxSund + TaxAntD + TaxUpCred]);
  lblTotalGST.Caption := Format('%m', [TaxFees + TaxDisb + TaxSund + TaxAntD + TaxUpCred]);

//  sgrTotals.Cells[0, 0] := Format('%m', [neFees.AsCurrency + TaxFees]);
  lblTotalFees.Caption  := Format('%m', [neFees.AsCurrency + TaxFees]);

//  sgrTotals.Cells[0, 1] := Format('%m', [neDisb.AsCurrency + TaxDisb]);
  lblDisbTotal.Caption  := Format('%m', [neDisb.AsCurrency + TaxDisb]);

//  sgrTotals.Cells[0, 2] := Format('%m', [neAntd.AsCurrency + TaxAntD]);
  lblAntdTotal.Caption  := Format('%m', [neAntd.AsCurrency + TaxAntD]);

//  sgrTotals.Cells[0, 3] := Format('%m', [neUpCred.AsCurrency + TaxUpCred]);
  lblCredTotal.Caption  := Format('%m', [neUpCred.AsCurrency + TaxUpCred]);

//  sgrTotals.Cells[0, 4] := Format('%m', [neSund.AsCurrency + TaxSund]);
  lblSundTotal.Caption  := Format('%m', [neSund.AsCurrency + TaxSund]);

  //sgrTotals.Cells[1, 0] := Format('%m', [neFees.AsCurrency + neFeesTaxFree.AsCurrency + TaxFees]);
  //sgrTotals.Cells[1, 1] := Format('%m', [neDisb.AsCurrency + neDisbTaxFree.AsCurrency + TaxDisb]);
  //sgrTotals.Cells[1, 2] := Format('%m', [neAntd.AsCurrency + neAntdTaxFree.AsCurrency + TaxAntD]);
  //sgrTotals.Cells[1, 3] := Format('%m', [neUpCred.AsCurrency + neUpCredTaxFree.AsCurrency + TaxUpCred]);
  //sgrTotals.Cells[1, 4] := Format('%m', [neSund.AsCurrency + neSundTaxFree.AsCurrency + TaxSund]);
...
{  sgrTotals.Cells[0, 6] := Format('%m', [neFees.AsCurrency +
                                         neDisb.AsCurrency +
                                         neAntd.AsCurrency +
                                         neUpCred.AsCurrency +
                                         neSund.AsCurrency]);
  ...
  lblTotalExGST.Caption := Format('%m', [neFees.AsCurrency +
                                         neDisb.AsCurrency +
                                         neAntd.AsCurrency +
                                         neUpCred.AsCurrency +
                                         neSund.AsCurrency]);

  lbltrust.Caption :=  Format('%m', [neTrust.AsCurrency * -1]);

//  sgrTotals.Cells[0, 7] := Format('%m', [neFees.AsCurrency + neDisb.AsCurrency + neAntd.AsCurrency + neUpCred.AsCurrency + neSund.AsCurrency - neTrust.AsCurrency + TaxFees + TaxDisb + TaxAntD + TaxUpCred + TaxSund]);

  if(neFees.AsCurrency + neDisb.AsCurrency + neAntd.AsCurrency + neUpCred.AsCurrency + neSund.AsCurrency - neTrust.AsCurrency + TaxFees + TaxDisb + TaxAntD + TaxUpCred + TaxSund) > 0 then
    lblTotalBill.Style.Font.Color := clBlue
  else
    lblTotalBill.Style.Font.Color := clRed;
  lblTotalBill.Caption := Format('%m', [neFees.AsCurrency + neDisb.AsCurrency +
                                       neAntd.AsCurrency + neUpCred.AsCurrency +
                                       neSund.AsCurrency {+ neFeesTaxFree.AsCurrency + neDisbTaxFree.AsCurrency +
                                       neAntdTaxFree.AsCurrency + neUpCredTaxFree.AsCurrency +
                                       neSundTaxFree.AsCurrency} - neTrust.AsCurrency + TaxFees +
                                       TaxDisb + TaxAntD + TaxUpCred + TaxSund + qryInvoice.FieldByName('discount').asCurrency]);
  iTotal := neFees.AsCurrency + neDisb.AsCurrency + neAntd.AsCurrency + neUpCred.AsCurrency +
            neSund.AsCurrency {+ neFeesTaxFree.AsCurrency + neDisbTaxFree.AsCurrency +
            neAntdTaxFree.AsCurrency + neUpCredTaxFree.AsCurrency + neSundTaxFree.AsCurrency } - neTrust.AsCurrency +
            TaxFees + TaxDisb + TaxAntD + TaxUpCred + TaxSund + qryInvoice.FieldByName('discount').asCurrency;
  lblDiscount.Caption := Format('%m',[qryInvoice.FieldByName('discount').asCurrency]);
end;
...
//  lvItems.Items.Clear;
   ...
   if ((rgFilter.ItemIndex = 0) or (rgFilter.ItemIndex = 1)) then
   begin
     if IsBillItem = 'N'  then
     begin
        if (IsBillHybrid = 'Y') then
            qryBillItems.SQL.Text := qryFeesHybrid.SQL.Text
        else
...
//        qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' and trunc(created) <= :interimdate ';
  ...
  if ((rgFilter.ItemIndex = 0) or (rgFilter.ItemIndex = 2)) then
  begin
    if qryBillItems.SQL.Text <> '' then
...
//      qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' and trunc(created) <= :interimdate ';
  ...
  if ((rgFilter.ItemIndex = 0) or (rgFilter.ItemIndex = 4)) then
  begin
   if qryBillItems.SQL.Text <> '' then
      qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' union all ';
...
//      qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' and trunc(created) <= :interimdate ';
  ...
  if ((rgFilter.ItemIndex = 0) or (rgFilter.ItemIndex = 3)) then
  begin
   if qryBillItems.SQL.Text <> '' then
      qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' union all ';
...
//      qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' and trunc(reqdate) <= :interimdate ';
  ...
  if ((rgFilter.ItemIndex = 0) or (rgFilter.ItemIndex = 5)) then
  begin
   if qryBillItems.SQL.Text <> '' then
      qryBillItems.SQL.Text := qryBillItems.SQL.Text + ' union all ';
...
{  while not qryFees.EOF do
  begin
    ...
    begin
      ...
      newListItem.SubItems.Add(Format('%m', [qryFees.FieldByName('AMOUNT').AsCurrency]));
      newListItem.SubItems.Add(Format('%m', [qryFees.FieldByName('TAX').AsCurrency]));
      New(ptrData);
...
{  while not qryDisb.EOF do
  ...
  begin
    ...
    newListItem.SubItems.Add(Format('%m', [0 - qryDisb.FieldByName('AMOUNT').AsCurrency]));
    newListItem.SubItems.Add(Format('%m', [0 - qryDisb.FieldByName('TAX').AsCurrency]));
...
//    newListItem.SubItems.Add('');
    ...
    newListItem.SubItems.Add(Format('%m', [qryAntd.FieldByName('AMOUNT').AsCurrency]));
    newListItem.SubItems.Add(Format('%m', [qryAntd.FieldByName('TAX').AsCurrency]));
    New(ptrData);
 ...
 { // Add all UnPaid Creditors to the Desktop
  ...
  begin
    ...
    begin
      ...
      newListItem.SubItems.Add(Format('%m', [0 - qryUpCred.FieldByName('AMOUNT').AsCurrency]));
      newListItem.SubItems.Add(Format('%m', [0 - qryUpCred.FieldByName('TAX').AsCurrency]));
      New(ptrData);
...
{  // Add all Sundries to the Desktop
  ...
  begin
    ...
    begin
      ...
      newListItem.SubItems.Add(Format('%m', [qrySund.FieldByName('AMOUNT').AsCurrency]));
      newListItem.SubItems.Add(Format('%m', [qrySund.FieldByName('TAX').AsCurrency]));
      New(ptrData);
...
//     qryBillItems.ParamByName('interimdate').AsDate := InterimDate;
   ...
   if (chkUnbilledTransactions.Visible = True) and (chkUnbilledTransactions.Checked = True) then
   begin
...
var subBill : TfrmSubBill;
iResult : integer;
begin
   if (boolean(cbSplitBill.EditValue) = True) then
   begin
      try
         subBill := TfrmSubBill.create(self);
         subBill.displaySubBill(qryInvoice.FieldByName('NMEMO').AsInteger);
...
begin
{
  Code Modified 22.11.2002 GG

  Wrapped all this up in some protection
  ...
  Self.Close;
   ...
   if (IsValidBillForMatter(qryInvoice.fieldByName('FILEID').AsString,
                            qryInvoice.fieldByName('BANK_ACCT').AsString) = True)  then
   begin
      qrySubBills.Close;
      qrySubBills.ParamByName('NMEMO').AsInteger := qryInvoice.fieldByName('NMEMO').AsInteger;
      qrySubBills.open;

      if qrySubBills.Eof and (boolean(cbSplitBill.EditValue) = True) then
      begin
         qrySubBills.close;
      ...
      if (not qrySubBills.Eof) and (boolean(cbSplitBill.EditValue) = False) then
      begin
         qrySubBills.close;
...
//               InvoiceMerge(qryInvoice.FieldByName('NMEMO').AsInteger, qryInvoice.FieldByName('NMATTER').AsInteger, True);
   ...
   begin
      ...
      MsgErr('This BILL does not have the same entity as the matter.'+ chr(13) +
             'You will need to delete this bill and create it under the correct entity.'+chr(13)+
...
{      if neFees.AsCurrency <> 0 then
      ...
      begin
         ...
         if ((CalculatedFeesTax + 0.01) < neFeesTax.AsCurrency) and
            (qryInvoice.FieldByName('DISPATCHED').AsString = '') and
            (neFees.AsCurrency <> 0.00) then
         begin
...
{         if edtDiscount.text <> '' then
         begin
		   ...
		   qryInvoice.Post;
             ...
             // post the sub bills
         if (SystemString('ALLOW_SPLIT_BILLS') = 'Y') and
            (qryInvoice.FieldByName('SPLIT_BILL').AsString = 'Y')   then
            dmAxiom.upDateSubBills(qryInvoice.fieldByName('NMEMO').AsInteger);
...
begin
   ...
   if (SystemString('ENFORCE_TRUST_REMIT_CRED') = 'Y') then
   begin
      if (TableCurrency('nmemo','nmemo',qryInvoice.FieldByName('NMEMO').AsInteger,'upcred') <> 0) then
      begin
         if (TableInteger('billtemplate','code',qryInvoice.FieldByName('BILLTEMPLATE').AsString, 'trust_use') = 0) then
         begin
            MsgErr('You have selected a format with a Office Remittance Section, however there are Creditors to bill on this format.  Please select a Trust format and try again.');
            bAttachBillTemplate := False;
         end;
      end;
   end;
   if bAttachBillTemplate then
   begin
      if IsBillItem = 'N' then
      begin
         procBillAddFees.ParamByName('p_NMemo').AsInteger := qryInvoice.FieldByName('NMEMO').AsInteger;
...
begin
  ...
  begin
    try
      ...
      ExecSQL;
      Close;
      SQL.Text := 'UPDATE NMEMO SET FEES = null, DISB = 0, ANTD = 0, SUND = 0, UPCRED = 0, FEES_FLAG = ''0'', DISB_FLAG = ''0'', ANTD_FLAG = ''0'', UPCRED_FLAG = ''0'', SUND_FLAG = ''0'' WHERE NMEMO = ' + sInvoice;
      ExecSQL;
      Close;
      dmAxiom.uniInsight.Commit;
    except
...
begin
   ...
   begin
      if MsgAsk('Do you want to reverse invoice ' + qryInvoice.FieldByName('REFNO').AsString + '?') = mrYes then
      begin
...
//   dtpInterim.Date := Trunc(Date);
 ...
 //  FLastSortColumn := lvItems.Columns[0];
   ...
   if (SystemString('ALLOW_SPLIT_BILLS') = 'Y') then
   begin
//      conSplitBills.Visible := ivAlways;
      cbSplitBill.Enabled := true;
   end
   else
   begin
//      conSplitBills.Visible := ivNever;
      cbSplitBill.Enabled := False;
   end;
...
begin
   if dtpInterim.Checked then dtpInterim.Date := Now;
end;
...
begin
   if FReturnSelection <> Nil then
   try
      if tvBillItems.DataController.RecordCount > 0 then
...
// 09/09/2009 changed test from one above to this one.  i think it should only do if it is a disb
      ...
      begin
         ...
         if Sender = tbtnUnassignDebtor then
   ...
   begin
      ...
      begin
         ...
         begin
            if (TableCurrency('nmemo','nmemo',qryInvoice.FieldByName('NMEMO').AsInteger,'upcred') <> 0) then
            begin
               if (TableInteger('billtemplate','code',qrySource.FieldByName('CODE').AsString, 'trust_use') = 0) then
               begin
                  MsgErr('You have selected a format with a Office Remittance Section, however there are Creditors to bill on this format.  Please select a Trust format');
                  bAttachBillTemplate := False;
            ...
            if (TableCurrency('nmemo','nmemo',qryInvoice.FieldByName('NMEMO').AsInteger,'upcred') = 0) then
            begin
               if (TableInteger('billtemplate','code',qrySource.FieldByName('CODE').AsString, 'trust_use') = 1) then
               begin
         ...
         begin
            ...
            if (IsBillItem <> sCurrIsBillItem) and (qryInvoice.FieldByName('DISPATCHED').IsNull) then
            begin
               RebuildData(Sender);
               DisplayItems;
            end;
...
begin
   ...
   if Sender is TdxBarButton then
      if MsgAsk('Remove Fees?') = mrNo then bRemove := False;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove Disbursements?') = mrNo then bRemove := False;
   if bRemove then
   begin
      RemoveDisbSund;
      RemoveDisb;
...
var
   dGstFree: double;
begin
   qryNew.SQL.Text := 'UPDATE ALLOC SET NMEMO = null WHERE NINVOICE IS NULL AND NMATTER = ' +
                       qryInvoice.FieldByName('NMATTER').AsString + ' AND NMEMO = ' + qryInvoice.FieldByName('NMEMO').AsString;
   qryNew.ExecSQL;
   qryNew.Close;
   neDisb.AsCurrency := 0;
   qryInvoice.Edit;
   ...
   TaxDisb := neDisbTax.AsCurrency;
   neDisbTaxFree.AsCurrency := dGstFree;
//   sgrTotals.Cells[0, 0] := Format('%m', [neDisbTax.AsCurrency]);
   lblDisbTotal.Caption := Format('%m', [neDisbTax.AsCurrency]);
end;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove Anticipateds?') = mrNo then bRemove := False;
...
var
   dGstFree: double;
begin
   qryNew.SQL.Text := 'UPDATE CHEQREQ SET NMEMO = null WHERE NMATTER = ' +
                      qryInvoice.FieldByName('NMATTER').AsString + ' AND NMEMO = ' + qryInvoice.FieldByName('NMEMO').AsString;
   qryNew.ExecSQL;
   qryNew.Close;
   neAntD.AsCurrency := 0;
   ...
   TaxAntD := neAntdTax.AsCurrency;
   neAntdTaxFree.AsCurrency := dGstFree;
//   sgrTotals.Cells[0, 0] := Format('%m', [neAntdTax.AsCurrency]);
   lblAntdTotal.Caption := Format('%m', [neAntdTax.AsCurrency]);
end;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove Sundry?') = mrNo then bRemove := False;

   if bRemove then
   begin
      qryNew.SQL.Text := 'DELETE FROM SUNDRY WHERE TYPE = ''ia'' AND NMATTER = ' +
                          qryInvoice.FieldByName('NMATTER').AsString + ' AND NMEMO = ' + qryInvoice.FieldByName('NMEMO').AsString;
      qryNew.ExecSQL;
      qryNew.Close;
      RemoveSund;
...
var
   dGstFree: double;
begin
   qryNew.SQL.Text := 'UPDATE SUNDRY SET NMEMO = null WHERE NMATTER = ' + qryInvoice.FieldByName('NMATTER').AsString +
                      ' AND NMEMO = ' + qryInvoice.FieldByName('NMEMO').AsString +
                      ' AND created_from_disb = ''N'' ';
   qryNew.ExecSQL;
   qryNew.Close;
   neSund.AsCurrency := 0;
   qryInvoice.Edit;
   ...
   TaxSund := neSundTax.Value;
   neSundTaxFree.AsCurrency := dGstFree;
//   sgrTotals.Cells[0, 0] := Format('%m', [neSundTax.Value]);
   lblSundTotal.Caption := Format('%m', [neSundTax.Value]);
end;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove Unpaid Creditors?') = mrNo then bRemove := False;
...
var
   dGstFree: double;
begin
      qryNew.SQL.Text := 'UPDATE ALLOC SET NMEMO = null WHERE NINVOICE IS NOT NULL AND NMATTER = ' + qryInvoice.FieldByName('NMATTER').AsString + ' AND NMEMO = ' + qryInvoice.FieldByName('NMEMO').AsString;
      qryNew.ExecSQL;
      qryNew.Close;
   ...
   TaxUpCred := neUpCredTax.AsCurrency;
   neUpCredTaxFree.AsCurrency := dGstFree;
//   sgrTotals.Cells[0, 0] := Format('%m', [neUpCredTax.AsCurrency]);
   lblCredTotal.Caption := Format('%m', [neUpCredTax.AsCurrency]);
end;
...
var
   dGstFree: double;
begin
   if (neDisb.AsCurrency = 0) and (qryInvoice.FieldByName('DISB').AsCurrency <> 0) then
      if MsgAsk('Do you want to remove all Disbursements from this bill?') = mrYes then
...
var
   dGstFree: double;
begin
   if (neAntd.AsCurrency = 0) and (qryInvoice.FieldByName('ANTD').AsCurrency <> 0) then
      if MsgAsk('Do you want to remove all Anticipated Disbursement from this bill?') = mrYes then
...
var
   dGstFree: double;
begin
   if (neSund.AsCurrency = 0) and (qryInvoice.FieldByName('SUND').AsCurrency <> 0) then
      if MsgAsk('Do you want to remove all Sundries from this bill?') = mrYes then
...
{  if neTrust.AsCurrency > (cTrustBal - cProtectedBal) then
   ...
   if neTrust.AsCurrency > (cTrustBal) then
...
begin
   bAttachBillTemplate := true;
   popGridRemoveUpCredClick(sender);
   SaveInvoice;
   if (SystemString('ENFORCE_TRUST_REMIT_CRED') = 'Y') then
   begin
      if (TableCurrency('nmemo','nmemo',qryInvoice.FieldByName('NMEMO').AsInteger,'upcred') <> 0) then
...
var
   dGstFree: double;
begin
   if (neUpCred.AsCurrency = 0) and (qryInvoice.FieldByName('UPCRED').AsCurrency <> 0) then
      if MsgAsk('Do you want to remove all unpaid Creditors from this bill?') = mrYes then
...
procedure TfrmInvoice.DoUplift(Sender: TObject; APercentage: Single);
var
  sDesc : String;
...
begin
   if (SystemString('memo_bill_default') = '') then
   begin
      qryPhonebook.Close;
...
procedure TfrmInvoice.ppLabel7GetText(Sender: TObject; var Text: String);
begin
   Text := qryInvoice.FieldByName('REFNO').AsString;
end;

procedure TfrmInvoice.ppLabel11GetText(Sender: TObject; var Text: String);
begin
   Text := qryMatter.FieldByName('Author').AsString + ':' + qryMatter.FieldByName('FILEID').AsString;
end;

procedure TfrmInvoice.ppLabel4GetText(Sender: TObject; var Text: String);
begin
...
//      FeeList := 'TO: ';
      While not eof do
      begin
         if (Feelist <> '') AND (not FirstTime) then
            FeeList := FeeList + '; ';
...
//      DisbList := 'TO: ';
      While not eof do
      begin
         if (DisbList <> '') AND (not FirstTime) then
            DisbList := DisbList + '; ';
...
procedure TfrmInvoice.ppLabel15GetText(Sender: TObject; var Text: String);
begin
...
procedure TfrmInvoice.ppDBText11GetText(Sender: TObject; var Text: String);
begin
   case StrToInt(Text) of
      1: Text := 'Professional Fees';
      2: Text := 'Disbursements';
      3: Text := 'Anticipated Disbursements';
      4: Text := 'Sundry Expenses';
      5: Text := 'Creditors';
   end;
end;

procedure TfrmInvoice.ppLabel28GetText(Sender: TObject; var Text: String);
begin
...
procedure TfrmInvoice.ppLabel25GetText(Sender: TObject; var Text: String);
begin
   if (qryInvoice.FieldByName('dispatched').IsNull) then
...
procedure TfrmInvoice.ppDBText1GetText(Sender: TObject; var Text: String);
begin
   Text := 'RE: ' + Text;
end;
...
procedure TfrmInvoice.neFeesKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
   dGstFree: double;
begin
...
begin
   ...
   try
      ...
      case tvBillItemsTYPE.EditValue of
         IMG_FEES:
            begin
               if dmAxiom.Security.Fee.Edit then
               begin
...
//            sgrTotals.Cells[0, 0] := Format('%m', [neFeesTax.AsCurrency]);
         ...
         if tvBillItemsTYPE.EditValue = IMG_SUND then
         begin
            neSundTax.Value := ShowTax(neSund.AsCurrency, dGstFree, 'SUNDRY',
                                     qryInvoice.FieldByName('FILEID').AsString, DefaultTax,
                                     qryInvoice.FieldByName('NMATTER').AsInteger,
               ...
               begin
                  ...
                  case tvBillItemsTYPE.EditValue of
                     ...
                     begin
                       ...
                       Close;
                       SQL.Text := 'UPDATE FEE SET TAXCODE = ''' + sTaxCode + ''', TAX = AMOUNT * :TaxRate WHERE NFEE = ' + IntToStr(tvBillItemsUNIQUEID.EditValue);
                     ...
                     begin
                       ...
                       Close;
                       SQL.Text := 'UPDATE ALLOC SET TAXCODE = ''' + sTaxCode + ''', TAX = AMOUNT * :TaxRate, BILLED_TAX_AMOUNT = AMOUNT * :TaxRate  WHERE NALLOC = ' + IntToStr(tvBillItemsUNIQUEID.EditValue);
                     ...
                     begin
                       ...
                       Close;
                       SQL.Text := 'UPDATE CHEQREQ SET TAXCODE = ''' + sTaxCode + ''', TAX = AMOUNT * :TaxRate WHERE NCHEQREQ = ' + IntToStr(tvBillItemsUNIQUEID.EditValue);
                     ...
                     begin
                       ...
                       Close;
                       SQL.Text := 'UPDATE SUNDRY SET TAXCODE = ''' + sTaxCode + ''', TAX = AMOUNT * :TaxRate WHERE NSUNDRY = ' + IntToStr(tvBillItemsUNIQUEID.EditValue);
...
begin
   ...
   begin
      ...
      bGrouped := False;
      if (AView.ViewData.GridView.GroupedItemCount > 0) then
      begin
         grdBillItems.FocusedView.DataController.Groups.FullExpand;
         ...
         try
            ...
            begin
               ...
               begin
                  ...
                  if not GroupRow.IsData then // Group or master row
                     inc(grpItemCount);
                  try
                     GoToNext(True);
                  except
      ...
      begin
         if (AView.ViewData.Records[iCtr].Level <> 0) or
            (not bGrouped ) then
         begin
            AView.ViewData.Records[iCtr].Focused := True;
            if AView.ViewData.GetRecordByIndex(iCtr).Values[0] = True then
...
//               ImageIndex := tvBillItemsTYPE.EditValue;
 ...
 //          lvBillItems.DataController.SetFocus;
               if (ImageIndex = IMG_UPCRED) then
                  qryNew.SQL.Text := 'UPDATE ALLOC SET NMEMO = null WHERE NALLOC = ' + string(AView.ViewData.GetRecordByIndex(iCtr).Values[13])  // string(tvBillItems.DataController.Values[iCtr, tvBillItemsUNIQUEID.Index])  // IntToStr(tvBillItemsUNIQUEID.EditValue)
               else if (ImageIndex = IMG_ANTD) then
                  qryNew.SQL.Text := 'UPDATE CHEQREQ SET NMEMO = null, BILLED = ''N'' WHERE NCHEQREQ = ' + string(AView.ViewData.GetRecordByIndex(iCtr).Values[13])  // string(tvBillItems.DataController.Values[iCtr, tvBillItemsUNIQUEID.Index])  // IntToStr(tvBillItemsUNIQUEID.EditValue)
               else
                  qryNew.SQL.Text := 'UPDATE ' + sTable + ' SET NMEMO = null WHERE N' + sTable + ' = ' + string(AView.ViewData.GetRecordByIndex(iCtr).Values[13]);  //IntToStr(tvBillItemsUNIQUEID.EditValue);
           ...
           // Take this amount off the invoice
               ...
               if ImageIndex = IMG_UPCRED then
                  qryNew.SQL.Text := 'SELECT AMOUNT FROM ALLOC WHERE NALLOC = ' + string(Aview.ViewData.GetRecordByIndex(iCtr).Values[13]) //IntToStr(tvBillItemsUNIQUEID.EditValue)
               else
               begin
                  if sTable = 'FEE' then
                  begin
                     if IsBillItem = 'N' then
                        ...
                        qryNew.SQL.Text := 'SELECT TASK_AMOUNT as AMOUNT FROM ' + sTable + ' WHERE N' + sTable + ' = ' + string(AView.ViewData.GetRecordByIndex(iCtr).Values[13]);  //IntToStr(tvBillItemsUNIQUEID.EditValue);
                  end
                  else
                     qryNew.SQL.Text := 'SELECT AMOUNT FROM ' + sTable + ' WHERE N' + sTable + ' = ' + string(AView.ViewData.GetRecordByIndex(iCtr).Values[13]);  //IntToStr(tvBillItemsUNIQUEID.EditValue);
               end;
...
begin
  qryNew.SQL.Text := 'UPDATE ALLOC SET NMEMO = null WHERE NINVOICE IS NULL AND NMATTER = ' +
                      IntToStr(qryInvoice.FieldByName('NMATTER').AsInteger) + ' AND NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger);
  qryNew.ExecSQL;
  qryNew.Close;
  neDisb.AsCurrency := 0;
  qryInvoice.Edit;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove Cheque Requisitions?') = mrNo then bRemove := False;
   // 11 Nov 2017 - set removed cheq req back to not billed
   if bRemove then
   begin
      qryNew.SQL.Text := 'UPDATE CHEQREQ SET NMEMO = null, BILLED = ''N'' WHERE NMATTER = ' +
                          qryInvoice.FieldByName('NMATTER').AsString + ' AND NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger);
      qryNew.ExecSQL;
      qryNew.Close;
      neAntD.AsCurrency := 0;
      qryInvoice.Edit;
...
begin
   ...
   if bRemove then
   begin
      qryNew.SQL.Text := 'UPDATE ALLOC SET NMEMO = null WHERE NINVOICE IS NOT NULL AND NMATTER = ' +
                          qryInvoice.FieldByName('NMATTER').AsString + ' AND NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger);
      qryNew.ExecSQL;
      qryNew.Close;
      neUpCred.AsCurrency := 0;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove Sundries?') = mrNo then bRemove := False;
   ...
   begin
      ...
      qryNew.SQL.Text := 'UPDATE SUNDRY SET NMEMO = null WHERE NMATTER = ' +
                          IntToStr(qryInvoice.FieldByName('NMATTER').AsInteger) + ' AND NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger);
      qryNew.ExecSQL;
      qryNew.Close;
      RemoveDisbSund;
...
begin
   bRemove := True;
   if Sender is TdxBarButton then
      if MsgAsk('Remove All Transactions?') = mrNo then bRemove := False;
...
//        popItemWriteOff.Enabled := True;
      ...
      if (ImageIndex = IMG_DISB) or
         (ImageIndex = IMG_UPCRED) then
   ...
   if ((bAlterGSTAmount) and (rgFilter.ItemIndex in [2,5]) and (qryBillItems.RecordCount > 0) and (qryInvoice.FieldByName('DISPATCHED').IsNull)) then
   begin
...
begin
   ...
   begin
      ...
      if SettingLoadInteger(C_BILLITEMSGRID,C_COLUMNWIDTH + IntToStr(LNumFields)) > 0 then
      ...
      if tvBillItems.Columns[LNumFields].Visible then
         ...
         begin
            if SettingLoadString(C_BILLITEMSGRID,C_COLUMNNAME + IntToStr(i)) = tvBillItems.Columns[LNumFields].Name then
               if SettingLoadInteger(C_BILLITEMSGRID, C_COLUMNPOSITION + IntToStr(i)) = 0 then
               begin
...
begin
    if (bAlterGSTAmount) then
    begin
      if (rgFilter.itemindex = 2) then
      Begin
          if TfrmAdjustBilledTax.AlterBilledTaxAmount(qryBillItems.FieldByName('UNIQUEID').AsInteger,'DISB') then
          Begin
              qryBillItems.Close;
              qryBillItems.Open;
          End;
      End
      else if (rgFilter.itemindex = 5) then
      begin
          if TfrmAdjustBilledTax.AlterBilledTaxAmount(qryBillItems.FieldByName('UNIQUEID').AsInteger,'SUND') then
          Begin
              qryBillItems.Close;
              qryBillItems.Open;
          End;
...
procedure TfrmInvoice.cxRichEdit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
                     ...
                     begin
                        ...
                        begin
                           ...
                           begin
                              ...
                              ParamByName('NFEE').AsInteger := Integer(Values[ARowInfo.RecordIndex, tvBillItemsUNIQUEID.Index]);    // dmFeeList.qFees.FieldByName('NFEE').AsInteger;
//
                              ExecSQL();
                              Close;
                           end;
...
begin
   ...
   begin
      ...
      begin
         Close;
         SQL.Text := 'select code, descr, rate from sundrytype, EXPENSE_TEMPLATE_LINK '+
                     'where expense_code = code and exp_template_code = ' + quotedstr(sExpTemplate);
         Open;
         while (eof = false) do
         begin
             SundryInsert(qryInvoice.FieldByName('NMATTER').AsInteger,
                          dmAxiom.qryTmp.FieldByName('code').AsString,
                          dmAxiom.qryTmp.FieldByName('descr').AsString,
                          dmAxiom.qryTmp.FieldByName('rate').AsCurrency, 1,
                          qryInvoice.FieldByName('NMEMO').AsInteger);
             Next;
         end;
      end;
      SaveInvoice;
      ...
      neSundTax.Value := ShowTax(neSund.AsCurrency, dGstFree, 'SUNDRY',
                                     ...
                                     qryInvoice.FieldByName('NMATTER').AsInteger,
                                     qryInvoice.FieldByName('NMEMO').AsInteger,
                                     qryInvoice.FieldByName('GENERATED').AsDateTime);
      TaxSund := neSundTax.Value;
      neSundTaxFree.AsCurrency := dGstFree;
//      sgrTotals.Cells[0, 0] := Format('%m', [neSundTax.Value]);
      lblSundTotal.Caption := Format('%m', [neSundTax.Value]);
...
begin
   ...
   begin
      ...
      begin
        ...
        edtDiscount.Text := FloatToStr(((neFees.AsCurrency + neFeesTax.AsCurrency) * (strtofloat(copy(edtDiscount.Text,1,length(edtDiscount.Text)-1))/100))*-1);
...
procedure TfrmInvoice.memoNotesKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   If Key = VK_SPACE then
      QuickCode(Sender);
...
procedure TfrmInvoice.popNotesKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   If Key = VK_SPACE then
      QuickCode(Sender);
...
procedure TfrmInvoice.memoNotesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
{   if Key = VK_Tab then
   begin
...
begin
  ...
  begin
     if (tvBillItemsTYPE.EditValue = IMG_FEES) and
        (qryInvoice.FieldByName('DISPATCHED').IsNull) then
     begin
...
procedure TfrmInvoice.tvBillItemsMouseDown(Sender: TObject;
...
begin
   if not qryInvoice.IsEmpty then
   begin
      if MsgAsk('Do you want to remove the discount from invoice ' + qryInvoice.FieldByName('REFNO').AsString + '?') = mrYes then
      begin
         bOKtoPost:= (PostIntoLockedPeriod(now) in [prNotLocked, prOKToProceed]);
         if bOKtoPost then
         begin
...
begin
   TdxBarButton(Sender).Lowered := (not TdxBarButton(Sender).Lowered) ;
   if (TdxBarButton(Sender).Lowered) then
      grdBillItems.FocusedView.DataController.Groups.FullExpand
   else
...
procedure TfrmInvoice.tvBillItems1DESCRPropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
   if tvBillItemsTYPE.EditValue = IMG_FEES then
   begin
...
procedure TfrmInvoice.tvBillItems2InitEdit(Sender: TcxCustomGridTableView;
  AItem: TcxCustomGridTableItem; AEdit: TcxCustomEdit);
begin
    // In this example, we'll do LiveSpelling in any in-place editing control
    // that comes up as a RichEdit.

    if (AEdit is TcxRichEdit) and dmAxiom.DictionaryInstalled then
    begin
...
procedure TfrmInvoice.tvBillItemsDESCRPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
      ...
      case tvBillItemsTYPE.EditValue of
         ...
         begin
            ...
            begin
               ...
               if LReturn <> mrCancel then
               begin
                  if LFeeNew.cmbMatterFind.Text <> qryInvoice.FieldByName('FILEID').AsString then
                  begin
...
procedure TfrmInvoice.tvBillItemsEditing(Sender: TcxCustomGridTableView;
  AItem: TcxCustomGridTableItem; var AAllow: Boolean);
begin
   if (qryInvoice.FieldByName('DISPATCHED').IsNull) then
      AAllow := True
...
begin
   qryNew.SQL.Text := 'DELETE FROM SUNDRY WHERE NMATTER = ' + IntToStr(qryInvoice.FieldByName('NMATTER').AsInteger) +
                      ' AND NMEMO = ' + IntToStr(qryInvoice.FieldByName('NMEMO').AsInteger) +
                      ' AND created_from_disb = ''Y'' ';
   qryNew.ExecSQL;
   qryNew.Close;
end;
...
begin
   try
      ...
      begin
         ...
         if fieldByname('INVOICE_COPY').IsNull then
            exit;

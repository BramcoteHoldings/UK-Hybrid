...
uses
  ...
  cxMemo, cxLookAndFeels, cxLookAndFeelPainters, cxNavigator, cxDataControllerConditionalFormattingRulesManagerDialog, cxContainer, cxGroupBox, cxRadioGroup,
  cxCurrencyEdit, cxTextEdit, dxDateRanges;
...
type
  TfrmSubBill = class(TForm)
    ...
    cxGridSubbillsPERCENTAGE: TcxGridDBColumn;
    ...
    cxGridSubbillsDBTableViewPAYABLEBY: TcxGridDBColumn;
    rgEntryType: TcxRadioGroup;
    ...
    procedure rgEntryTypeClick(Sender: TObject);
    procedure cxGridSubbillsTOTALPropertiesValidate(Sender: TObject; var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
  private
  ...
  procedure displaySubBill(iNmemo : integer; BillTotal: Currency);
...
procedure TfrmSubBill.cxGridSubbillsTOTALPropertiesValidate(Sender: TObject; var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  try
    //cxGridSubBillsPERCENTAGE.EditValue := Currency(DisplayValue) * 100 / cBillTotal;
    qrySubBills.FieldByName('PERCENTAGE').AsFloat := Currency(DisplayValue) * 100 / cBillTotal;
    qrySubBills.FieldByName('TOTAL').AsCurrency := strtocurr(DisplayValue);
//    Error := True;
  Except
//    Error := False;
  end;
...
procedure TfrmSubBill.displaySubBill(iNmemo : integer; BillTotal: Currency);
begin
   self.iNmemo := iNmemo;
   self.cBillTotal := BillTotal;
   ...
   begin
      ...
      begin
        cxGridSubBillsTOTAL.EditValue := cxGridSubBillsPERCENTAGE.EditValue * cBillTotal/100.00;
...
begin
   ...
   begin
      ...
      cTotal := cTotal + qrySubBills.fieldByName('TOTAL').AsFloat;
      qrySubBills.Next;
   end;
   ...
   if rgEntryType.ItemIndex = 0 then
   begin
   ...
   begin
     lblTotalPercent.Caption := Format('%10.2f', [cTotal]);
     lblTotalPercentUn.Caption := Format('%10.2f', [cBillTotal - cTotal]);
   end;

   if abs(iPercent - 100) > 0.00000000001 then
   begin
...
procedure TfrmSubBill.rgEntryTypeClick(Sender: TObject);
begin
    ...
    begin
      ...
      begin
        ...
        cxGridSubbillsPERCENTAGE.Visible := True;
      end
      else
      begin
        cxGridSubbillsTOTAL.Visible := True;
        cxGridSubbillsPERCENTAGE.Visible := False;
      end;

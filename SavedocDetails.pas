...
uses
  ...
  JvExControls, JvLabel, OleServer, RwMAPI_TLB, uRwMapiV3Session,
  cxLookAndFeels;
...
type
  ...
  private
    ...
    function WriteFileDetailsToDB(ANewDocName: string): boolean;
  public
    { Public declarations }
...
uses
    MatterSearch, savedoc, SaveDocFunc, MapiUtil, MapiTags,   uRwSysUtils
  , uRwMapiTagsH
  , uRwDateTimeUtils;
...
//   Application.CreateForm(TdmSaveDoc, dmSaveDoc);
   try
      ...
      begin
         ...
         lblPath.Caption := SystemString('DRAG_DEFAULT_DIRECTORY');
...
var
   ...
   POldName, AExt, ADispName: string;
begin
   SaveDocument := False;
   ADocumentSaved := True;
   VarDocName := '';
...
//                        MapiSession.LogonInfo.ShowLogonDialog    := True;
                  ...
                  begin
                     AttTable := FSavedMsg.GetAttachmentTable;
                     AttTable.SetFields(VarArrayOf([PR_ATTACH_NUM]));
                     AttTable.First;
                     while not AttTable.EOF do
                     begin
                        Attachment := FSavedMsg.OpenAttachment(AttTable.FieldByName(PR_ATTACH_NUM).AsInteger, alReadOnly, False);
                        ...
                        WriteFileDetailsToDB(VarDocName);
                        AttTable.Next;
                     end;
                  end;
                  OldDocName := POldName;
...
begin
   case AButtonIndex of
      ...
      1: lblPath.Caption := SystemString('DRAG_DEFAULT_DIRECTORY');   // btnTxtDocPath.Text := SystemString('DRAG_DEFAULT_DIRECTORY');
   end;
end;
...
begin
   ...
   begin
      ...
      lDocName := ExtractFileName(DocName);
      // clean up subject line
      for x := 0 to length(lDocName) do
      begin
         if (lDocName[x] in ['/','\','?','"','<','>','|','*',':',';','&', '!']) then
            lDocName[x] := ' ';
      end;
   ...
   cmbAuthor.EditValue := TableString('EMPLOYEE','USER_NAME',UpperCase(dmSaveDoc.orsAxiom.Username), 'CODE');
...
function TfrmSaveDocDetails.WriteFileDetailsToDB(ANewDocName: string): boolean;
var
   FileExt: string;
   FileImg: integer;
begin

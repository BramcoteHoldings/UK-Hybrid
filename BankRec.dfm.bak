object frmBankRec: TfrmBankRec
  ...
  object qryAdjustments: TUniQuery
    Connection = dmAxiom.uniInsight
    SQL.Strings = (
      'SELECT NVL (SUM (amount), 0) * -1 AS adj_total'
      '  FROM ('
      ...
      '  WHERE acct = :p_acct'
        ...
        '      AND CASE WHEN ((:trust = '#39'T'#39') AND (:trustasoffice = '#39'N'#39') A' +
        'ND (TRUNC(system_date) <= :p_dateto) '
      
        '                      AND (TRUNC(presented) > :p_dateto OR prese' +
        'nted IS NULL AND bankrec_saved = '#39'N'#39')) THEN 1'
      
        '               WHEN ((:trust = '#39'T'#39') AND (:trustasoffice = '#39'Y'#39') A' +
        'ND (TRUNC (created) <= :p_dateto)'
      ...
      '  UNION ALL'
        ...
        'n the period, not reconciled or renciled after period (reduce ba' +
        'lance)'
      
        '  SELECT '#39'DEPOSIT'#39' AS grp, bankdep.deposit_date, receipt.descr, ' +
        'bankdep.nbankdep || '#39' '#39' AS refno, receipt.TYPE, receipt.payor, r' +
        'eceipt.amount, '#39'dep1'#39' AS subx'
      '  FROM bankdep, receipt'
      
        '  WHERE bankdep.acct = :p_acct --AND bankdep.acct = receipt.acct' +
        ' AND bankdep.nbankdep = receipt.nbankdep'
      
        '      AND CASE WHEN (:trustasoffice = '#39'N'#39') AND TRUNC (receipt.sy' +
        'stem_date) <= :p_dateto THEN 1 -- this was missing an equal sign'
      
        '               WHEN (:trustasoffice = '#39'Y'#39') AND TRUNC (receipt.cr' +
        'eated) <= :p_dateto THEN 1'
      '          END = 0 -- this was one'
      ...
      '  UNION ALL'
        ...
        'd in the period, not reconciled or renciled after period (reduce' +
        ' balance)'
      ...
      '  FROM bankdep, receipt'
        ...
        '      AND TRUNC (receipt.created) <= :p_dateto --AND bankdep.las' +
        't_nreceipt = receipt.nreceipt '
      '      AND (:trust <> '#39'T'#39')'
      ...
      '  FROM bankdep, receipt'
        ...
        '      AND TRUNC(receipt.system_date) < :p_dateto AND TRUNC(bankd' +
        'ep.deposit_date) >= :p_dateto'
      
        '      AND TRUNC(bankdep.recondate) < :p_dateto AND bankdep.bankr' +
        'ec_saved = '#39'N'#39
      '  UNION ALL'
      ...
      '  FROM bankdep, receipt'
        ...
        '  WHERE bankdep.acct = :p_acct AND bankdep.acct = receipt.acct A' +
        'ND bankdep.nbankdep = receipt.nbankdep AND TRUNC(receipt.system_' +
        'date) >= :p_dateto'
      ...
      '  UNION ALL'
        ...
        '  --- deposited receipts - deposited in period, reconciled after' +
        ' period'
      ...
      '  WHERE receipt.acct = :p_acct'
        ...
        '                WHEN ((:trust = '#39'T'#39') AND (:trustasoffice = '#39'Y'#39') ' +
        'AND TRUNC(receipt.created) <= :p_dateto AND receipt.banked = '#39'N'#39 +
        ') THEN 1'
      ...
      '        UNION ALL*/'
        ...
        '   --- deposited receipts - keyed receipt after period, not depo' +
        'sited or deposited in period (bit funny)'
      
        '   SELECT '#39'RECEIPT1'#39' AS grp, receipt.created, receipt.descr, rec' +
        'eipt.rcptno AS refno, receipt.TYPE, receipt.payor,(0 - receipt.a' +
        'mount) AS amount, '#39'rec3'#39' AS subx'
      '   FROM bankdep, receipt'
      ...
      '       AND CASE WHEN (:trust = '#39'T'#39') AND (:trustasoffice = '#39'N'#39')'
        ...
        '                  AND (bankdep.deposit_date IS NULL OR ((TRUNC(b' +
        'ankdep.deposit_date) <= :p_dateto) AND TRUNC(bankdep.system_date' +
        ') > :p_dateto)) THEN 1'
      '                WHEN (:trust = '#39'T'#39') AND (:trustasoffice = '#39'Y'#39') '
      
        '                    AND (bankdep.deposit_date IS NULL OR ((TRUNC' +
        '(bankdep.deposit_date) <= :p_dateto) AND TRUNC(bankdep.deposit_d' +
        'ate) > :p_dateto)) THEN 1'
      '           END = 1'
      '  UNION ALL'

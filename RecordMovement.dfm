object frmRecordMovement: TfrmRecordMovement
  Left = 335
  Top = 276
  Caption = 'Movement History'
  ClientHeight = 446
  ClientWidth = 746
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Segoe UI'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 106
  TextHeight = 17
  object grdRecordMovement: TcxGrid
    Left = 0
    Top = 0
    Width = 746
    Height = 411
    Align = alClient
    TabOrder = 0
    LookAndFeel.NativeStyle = True
    ExplicitTop = -2
    object tvRecordMovement: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      DataController.DataSource = dsRecMovement
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      OptionsBehavior.CellHints = True
      OptionsBehavior.CopyCaptionsToClipboard = False
      OptionsBehavior.ColumnHeaderHints = False
      OptionsCustomize.ColumnFiltering = False
      OptionsData.Deleting = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsSelection.CellSelect = False
      OptionsView.CellEndEllipsis = True
      OptionsView.NavigatorOffset = 54
      OptionsView.GroupByBox = False
      OptionsView.IndicatorWidth = 13
      Preview.LeftIndent = 21
      object tvRecordMovementCREATED: TcxGridDBColumn
        Caption = 'Modified Date'
        DataBinding.FieldName = 'CREATED'
        MinWidth = 21
        Width = 121
      end
      object tvRecordMovementDATE_RELEASED: TcxGridDBColumn
        Caption = 'Release'
        DataBinding.FieldName = 'DATE_RELEASED'
        MinWidth = 21
        Width = 109
      end
      object tvRecordMovementRELEASED_TO: TcxGridDBColumn
        Caption = 'Released To'
        DataBinding.FieldName = 'RELEASED_TO'
        MinWidth = 21
        Width = 116
      end
      object tvRecordMovementDATE_RETURNED: TcxGridDBColumn
        Caption = 'Returned'
        DataBinding.FieldName = 'DATE_RETURNED'
        MinWidth = 21
        Width = 115
      end
      object tvRecordMovementMOVEMENT_REASON: TcxGridDBColumn
        Caption = 'Reason'
        DataBinding.FieldName = 'MOVEMENT_REASON'
        MinWidth = 21
        Width = 236
      end
      object tvRecordMovementDESCR: TcxGridDBColumn
        Caption = 'Description'
        DataBinding.FieldName = 'DESCR'
        MinWidth = 21
        Width = 460
      end
      object tvRecordMovementMOD_BY: TcxGridDBColumn
        Caption = 'Modified By'
        DataBinding.FieldName = 'MOD_BY'
        MinWidth = 21
      end
      object tvRecordMovementDOCUMENT_ATTACHED: TcxGridDBColumn
        Caption = 'Document'
        DataBinding.FieldName = 'DOCUMENT_ATTACHED'
        PropertiesClassName = 'TcxCheckBoxProperties'
        Properties.NullStyle = nssUnchecked
        Properties.ValueChecked = 'Y'
        Properties.ValueUnchecked = 'N'
        MinWidth = 64
        Width = 64
      end
      object tvRecordMovementMOVEMENT_ACTION: TcxGridDBColumn
        Caption = 'Action'
        DataBinding.FieldName = 'MOVEMENT_ACTION'
        MinWidth = 21
      end
    end
    object grdRecordMovementLevel1: TcxGridLevel
      GridView = tvRecordMovement
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 411
    Width = 746
    Height = 35
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 1
    ExplicitTop = 370
    DesignSize = (
      746
      35)
    object cxButton1: TcxButton
      Left = 659
      Top = 4
      Width = 80
      Height = 27
      Anchors = [akRight, akBottom]
      Caption = 'Close'
      LookAndFeel.NativeStyle = True
      OptionsImage.Glyph.SourceDPI = 96
      OptionsImage.Glyph.Data = {
        
        F81EFFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
      TabOrder = 0
      OnClick = cxButton1Click
    end
  end
  object qryRecMovement: TUniQuery
    Connection = dmAxiom.uniInsight
    SQL.Strings = (
      'select * from'
      'record_movement'
      'where ndocument = :ndocument'
      'order by NRECORDMNT')
    Left = 52
    Top = 48
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ndocument'
        Value = nil
      end>
  end
  object dsRecMovement: TUniDataSource
    DataSet = qryRecMovement
    Left = 147
    Top = 49
  end
end

object dmDashboardInd: TdmDashboardInd
  ...
  object qryChargeSummary: TUniQuery
    ...
    SQL.Strings = (
        ...
        'sum(units)) units, 4 as eventtype from fee'
      'where author = :author'
      ...
      'union'
        ...
        'har(sum(units)) units, 3 as eventtype from fee'
      'where author = :author'
      ...
      'union'
        ...
        'um(units)) units, 2 as eventtype from fee'
      'where author = :author'
  ...
  object qryFeesListBudget: TUniQuery
    ...
    SQL.Strings = (
        ...
        'SELECT   disp_period, fees, fees_budget, nvl(a.period, b.period)' +
        ' period'
      
        ',trunc(nvl(fees,1)/ case when (nvl(fees_budget,1) = 0) then 1 el' +
        'se nvl(fees_budget,1) *100 end) as ratio'
      
        '    FROM (SELECT   SUM (amount) AS fees, TO_CHAR (invoicedate, '#39 +
        'Mon-YY'#39') as disp_period,'
      '                   get_fin_year (invoicedate),'
      ...
      '                          '#39'mm-yy'#39
        ...
        'd_date) a,'
      '         (SELECT   SUM (billedfees) fees_budget, period'
      '              FROM budget'
      '             WHERE employee = :author'
  ...
  object qryFeesReceivedGrid: TUniQuery
    ...
    SQL.Strings = (
        ...
        'SELECT   disp_period, fees_paid,fees_budget, nvl(a.period, b.per' +
        'iod) period'
      
        ',trunc(nvl(fees_paid,1)/ case when (nvl(fees_budget,1) = 0) then' +
        ' 1 else nvl(fees_budget,1) *100 end) as ratio'

...
uses
  ...
  cxLookAndFeels, cxGridExportLink, dxPSGlbl, dxPSUtl, dxPSEngn, dxPrnPg,
  ...
  dxPScxEditorProducers, dxPScxExtEditorProducers, dxPScxSchedulerLnk,
  cxNavigator, cxDataControllerConditionalFormattingRulesManagerDialog, Vcl.ExtCtrls;

type
  TfrmUnbilledWIP = class(TForm)
...
procedure TfrmUnbilledWIP.btnCloseClick(Sender: TObject);
begin
   Close;
end;


procedure TfrmUnbilledWIP.FormShow(Sender: TObject);
begin
...
procedure TfrmUnbilledWIP.FormClose(Sender: TObject;
...
function TfrmUnbilledWIP.GetGroupDisplayText(APeriod: integer): string;
var
   LPeriod, adj: integer;
   LYear: word;
begin
...
procedure TfrmUnbilledWIP.tvFeesBilledFullSORT_ORDERGetDisplayText(
  Sender: TcxCustomGridTableItem; ARecord: TcxCustomGridRecord;
  var AText: String);
begin
...
procedure TfrmUnbilledWIP.ExporttoExcel1Click(Sender: TObject);
var
   filename: string;
begin
...
procedure TfrmUnbilledWIP.Print1Click(Sender: TObject);
begin
   dxComponentPrinter1.Preview();
end;

procedure TfrmUnbilledWIP.cxButton1Click(Sender: TObject);
begin
  dxComponentPrinter1.Preview();
end;

procedure TfrmUnbilledWIP.cxButton2Click(Sender: TObject);
var
   filename: string;
begin
   if OpenDialog.Execute then
